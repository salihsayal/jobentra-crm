    ...
    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <div style={{ flex: 1 }}>
            <CustomLegend data={pieData} colors={CHART_COLORS} />
          </div>
        </div>
      </div>
        ...
        <ResponsiveContainer width="100%" height={260}>
          <BarChart data={barData} margin={{ top: 4, right: 4, left: -8, bottom: 0 }}>
            ...
            <XAxis
              ...
              tickLine={false}
              interval={0}
              angle={barData && barData.length > 6 ? -30 : 0}
              textAnchor={barData && barData.length > 6 ? 'end' : 'middle'}
              height={barData && barData.length > 6 ? 50 : 30}
            />
            <YAxis
              tick={{ fontSize: 11, fill: 'var(--text-dim)' }}
              axisLine={false}
