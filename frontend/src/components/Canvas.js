  ...
  if (currentView === 'search') {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        {omniResults.candidates?.length > 0 && <div><h3 style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-main)', marginBottom: 8 }}>Gefundene Kandidaten ({omniResults.candidates.length})</h3><DataTable data={omniResults.candidates} columns={[{ key: 'firstName', label: 'Vorname' },{ key: 'lastName', label: 'Nachname' },{ key: 'email', label: 'Email' },{ key: 'phone', label: 'Telefon' },{ key: 'status', label: 'Status', render: v => <StatusBadge status={v} /> }]} onRowClick={r => onOmniSelect('candidate', r)} /></div>}
