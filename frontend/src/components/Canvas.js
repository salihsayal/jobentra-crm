  ...
  if (currentView === 'search') {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        {omniResults.candidates?.length > 0 && <div><h3 style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-main)', marginBottom: 8 }}>Gefundene Kandidaten ({omniResults.candidates.length})</h3><DataTable data={omniResults.candidates} columns={[{ key: 'firstName', label: 'Name & Beruf', render: (v, r) => <div><div style={{ fontWeight: 600 }}>{r.firstName} {r.lastName}</div><div style={{ fontSize: 11, color: 'var(--text-dim)', marginTop: 1 }}>{r.job || '-'}</div></div> },{ key: 'skills', label: 'Fähigkeiten', render: (v) => {
          const { shown, hidden } = truncateSkills(v);
          if (!shown) return '-';
          return <span style={{ fontSize: 12, whiteSpace: 'nowrap' }}>{shown}{hidden > 0 && <span style={{ marginLeft: 6, padding: '1px 6px', borderRadius: 999, fontSize: 10, fontWeight: 600, background: 'var(--accent-light)', color: 'var(--accent)' }}>+{hidden}</span>}</span>;
        } },{ key: 'location', label: 'Ort' },{ key: 'status', label: 'Status', render: v => <StatusBadge status={v} /> }]} onRowClick={r => onOmniSelect('candidate', r)} /></div>}
